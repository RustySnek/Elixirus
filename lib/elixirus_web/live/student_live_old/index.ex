defmodule ElixirusWeb.StudentLiveOld.Index do
  require Logger
  ...
  def fetch_data(socket, token, semester) do
    ...
    |> create_fetcher(unread_messages, :unread_messages, fn ->
      {SnakeArgs.from_params(:fetchers, :fetch_all_messages, [token])
       |> python!(python_timeout: :infinity), nil}
    end)
    |> create_fetcher(attendance, :new_attendance, fn ->
      {SnakeArgs.from_params(:fetchers, :fetch_new_attendance, [token, semester])
       |> python!(python_timeout: :infinity), semester}
    end)
    |> create_fetcher(new_grades, :new_grades, fn ->
      {SnakeArgs.from_params(:fetchers, :fetch_new_grades, [token, semester])
       |> python!(python_timeout: :infinity), semester}
    end)
    |> create_fetcher(
      student_data,
      :student_data,
      fn ->
        {SnakeArgs.from_params(:fetchers, :fetch_student_data, [token])
         |> python!(python_timeout: :infinity), nil}
      end
    )
    |> create_fetcher(schedule, :schedule, fn ->
      {SnakeArgs.from_params(:fetchers, :fetch_schedule, [token, year, month])
       |> python!(python_timeout: :infinity), year, month}
    end)
    |> create_fetcher(timetable, :timetable, fn ->
      {SnakeArgs.from_params(:overview, :handle_overview_timetable, [
         token,
